...
local function CancelCall()
    ...
    TriggerServerEvent('sequtest:server:CancelCall', GetPlayerServerId(PlayerId()), PhoneData.CallData)
    if PhoneData.CallData.CallType == "ongoing" then
        --exports['saltychat']:RemovePlayerFromCall(PhoneData.CallData.CallId)
...
local function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        TriggerServerEvent('sequtest:server:AnswerCall', GetPlayerServerId(PlayerId()), PhoneData.CallData)
...
RegisterCommand('phone', function()
    ...
    if not PhoneData.isOpen and LocalPlayer.state.isLoggedIn then
        if not PlayerData.metadata['ishandcuffed'] and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] and not IsPauseMenuActive() then
            ...
            QBCore.Functions.Notify("Geçersiz eylem", "error")
        end
 end
end)

RegisterKeyMapping('phone', 'Telefon', 'keyboard', 'F1')
...
RegisterNUICallback('startwalk', function()
    SetNuiFocusKeepInput(true)
end)

RegisterNUICallback('stopwalk', function()
...
--         QBCore.Functions.Notify("Aracınız GPS'e işaretlendi.", "success")
--     else
--         QBCore.Functions.Notify("Aracınız bulunamadı", "error")
--     end
-- end)
...
RegisterNUICallback('track-vehicle', function(data, cb)
    local veh = data.veh
    if findVehFromPlateAndLocate(veh.plate) then
        QBCore.Functions.Notify("Aracınız konuma işaretlendi", "success")
    else
        QBCore.Functions.Notify("Aracınız GPS üzerinde bulunamadı", "error")
    end
end)
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    QBCore.Functions.Notify('GPS Ayarlandı: '..data.alert.title)
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
		...
		SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { timeout=3000, title = "Banka", text = "Hesap mevcut değil!", icon = "fas fa-university", color = "#ff0000", }, })
...
RegisterNetEvent('qb-phone:client:TransferMoney', function(amount, newmoney)
    ...
    SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Banka", text = "&#36;"..amount.." hesabınıza eklendi!", icon = "fas fa-university", color = "#8c7ae6", }, })
...
RegisterNetEvent('qb-phone:client:UpdateAdverts', function(Adverts, LastAd)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Sahibinden",
            text = "Yeni ilan yayınlandı "..LastAd,
            icon = "fas fa-ad",
            color = "#ff8f1a",
            timeout = 2500,
        },
...
RegisterNetEvent('qb-phone:client:BillingEmail', function(data, paid, name)
    if paid then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            ...
            message = 'Faturayı Ödedi: '..name..' Tutar: $'..data.amount,
        })
    else
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Fatura Departmanı',
            subject = 'Fatura Ödenmedi!!',
            message = 'Fatura Reddedildi: '..name..' Tutar: $'..data.amount,
        })
    end
end)
        ...
        if PhoneData.isOpen then
            if SenderNumber ~= PhoneData.PlayerData.charinfo.phone then
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Whatsapp",
                        text = "Yeni mesaj "..IsNumberInContacts(SenderNumber).."!",
                        icon = "fab fa-whatsapp",
                        color = "#25D366",
                        timeout = 1500,
                    },
...
RegisterNetEvent('qb-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Banka",
                text = "$"..amount.." tutarında hesabınızdan para çekildi!",
                icon = "fas fa-university",
                color = "#ff002f",
                timeout = 3500,
            },
...
RegisterNUICallback('phone-silent-button', function(data,cb)
    if CallVolume == tonumber("0.2") then
        CallVolume = 0
        QBCore.Functions.Notify("Sessiz Modu Açık", "success")
        cb(true)
    else
        CallVolume = 0.2
        QBCore.Functions.Notify("Sessiz Modu Kapalı", "error")
        cb(false)
    end
end)
...
RegisterNetEvent('qb-phone:client:wenmophonebanknotify', function(amount,name)
    PhoneData.Adverts = Adverts
    SendNUIMessage({
        action = "PhoneNotification",
        PhoneNotify = {
            title = "Banka",
            text = "Hesabınıza para yattı  $" ..amount.. " Yatıran: "..name,
...
RegisterNetEvent('qb-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
    ...
    SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Bir Tweette sizden bahsedildi!", text = TweetMessage.message, icon = "fab fa-twitter", color = "#1DA1F2", }, })
...
RegisterNUICallback('CasinoPhoneJobCenter', function(data)
    if data.action == 2 then
        ...
        QBCore.Functions.Notify('Konum İşaretlendi', "success")
    end
end)
...
RegisterNUICallback('GetCar', function(data) --- Fatura versiyon
    ...
    if yolda then
        TriggerEvent('qb-phone:client:GarageNotify', "Zaten aktif vale hizmetin var.", 2000)
        return
    end
    if ValetTime then
    ...
    QBCore.Functions.TriggerCallback('qb-phone:server:GetInvoicesAll', function(fatura)
        if fatura < 50 then ------ 
            QBCore.Functions.TriggerCallback('qb-phone:server:GetVehicleByPlate', function(result)
                for k, v in pairs(result) do
                    if v.state == 1 then
                        ...
                        TriggerEvent('qb-phone:client:GarageNotify', "Bu aracı çıkartamazsın!!", 2000)
                    end
                    elseif v.state == 0 then
                        TriggerEvent('qb-phone:client:GarageNotify', "Aracın zaten dışarıda veya çekilmişlerde.", 2000)
            ...
            TriggerEvent('qb-phone:client:GarageNotify', "50$ tutarından fazla toplam faturanız var. Ödeyiniz", 3000)
        end
    end)
end)
