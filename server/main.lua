...
local WebHook = "https://discord.com/api/webhooks/1019667115415965778/QqUkxslw8BrQDB-huX-TCHUj12Jdrbuc_prrwCHOlEE_OtF8xkpLknUFKMFQR2hWRK3O"
...
QBCore.Functions.CreateCallback('qb-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    ...
    -- if not SenderPly and Config.BillingCommissions[society] then
        invoiceMailData = {
            sender = 'Fatura Departmanı',
            subject = 'Fatura Ödendi',
            message = string.format('%s %s Fatura Ödedi $%s', Ply.PlayerData.charinfo.firstname, Ply.PlayerData.charinfo.lastname, amount)
        }
    -- end
    --print(invoiceMailData)
...
QBCore.Functions.CreateCallback('qb-phone:server:ScanPlate', function(source, cb, plate)
    ...
    if plate ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, 'Yakınında araç yok', 'error')
        cb(nil)
    end
end)
...
QBCore.Functions.CreateCallback('qb-phone:server:GetGarageVehicles', function(source, cb)
    ...
    if result[1] ~= nil then
        for k, v in pairs(result) do
            ...
            local VehicleGarage = "Bulunamadı"
            ...
            local VehicleState = "Garaj"
            if v.state == 0 then
                VehicleState = "Dışarıda"
            elseif v.state == 2 then
                VehicleState = "Çekilmiş"
            end
...
RegisterNetEvent('qb-phone:server:wenmo_givemoney_toID', function(data)
    ...
    if OtherPly then
        ...
        local othcid = OtherPly.PlayerData.citizenid
    if plycid ~= othcid then 
        if (Ply.PlayerData.money.bank - amount) >= 0 then
            ...
            TriggerClientEvent('qb-phone:client:wenmophonebanknotify', OtherPly.PlayerData.source, amount, name)
        else
            TriggerClientEvent('QBCore:Notify', src, 'Yeteri kadar paran yok', 'error')
        end
    else
        TriggerClientEvent('QBCore:Notify', src, 'Kendine para gönderemezsin', 'error')
        end
    else
        TriggerClientEvent('QBCore:Notify', src, 'Kullanıcı aktif değil', 'error')
    end
end)
...
RegisterNetEvent('qb-phone:server:TransferMoney', function(iban, amount)
    ...
    if result[1] ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Bu hesap numarası mevcut değil! ", "error")
    end
end)
...
RegisterNetEvent('sequtest:server:CancelCall', function(Ply, tply)
    local TPlayer = QBCore.Functions.GetPlayerByPhone(tply.TargetData.number)

    local TPlayer_source = QBCore.Functions.GetSource(TPlayer.PlayerData.license)

    --print("CancelCall-tply[CallId]: " .. tostring(tply["CallId"]))
    --print("CancelCall-Ply: " .. Ply)
    --print("CancelCall-TPlayer_source: " .. TPlayer_source)

    if Ply ~= nil then
        if TPlayer ~= nil then
            exports['saltychat']:RemovePlayerFromCall(tostring(tply["CallId"]), Ply)
            exports['saltychat']:RemovePlayerFromCall(tostring(tply["CallId"]), TPlayer_source)
        end
    end
end)

RegisterNetEvent('sequtest:server:AnswerCall', function(Ply, tply)
    local TPlayer = QBCore.Functions.GetPlayerByPhone(tply.TargetData.number)
    
    local TPlayer_source = QBCore.Functions.GetSource(TPlayer.PlayerData.license)

    --print("AnswerCall-tply[CallId]: " .. tostring(tply["CallId"]))
    --print("AnswerCall-Ply: " .. Ply)
    --print("AnswerCall-TPlayer_source: " .. TPlayer_source)
	
    if Ply ~= nil then
        if TPlayer ~= nil then
            exports['saltychat']:AddPlayerToCall(tostring(tply["CallId"]), Ply)
            exports['saltychat']:AddPlayerToCall(tostring(tply["CallId"]), TPlayer_source)
        end
    end
end)
...
RegisterNetEvent("um-billing:server:bill", function(billed,amount)
    local biller = QBCore.Functions.GetPlayer(source)
    local billed = QBCore.Functions.GetPlayer(billed)
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    TriggerClientEvent('QBCore:Notify', source, 'Fatura kesildi', 'success')
                    TriggerClientEvent('QBCore:Notify', billed.PlayerData.source, 'Tarafınıza fatura kesildi!')
                else
                    TriggerClientEvent('QBCore:Notify', source, '0 dan yüksek bir tutar girin', 'error')
                end
            else
                TriggerClientEvent('QBCore:Notify', source, 'Kendinize fatura kesemezsiniz', 'error')
            end
        else
            TriggerClientEvent('QBCore:Notify', source, 'Kişi şehirde değil', 'error')
        end
end)
...
QBCore.Commands.Add('bill', 'Bill A Player', {{name = 'id', help = 'Player ID'}, {name = 'amount', help = 'Fine Amount'}}, false, function(source, args)
    ...
    if biller.PlayerData.job.name == "police" or biller.PlayerData.job.name == 'ambulance' or biller.PlayerData.job.name == 'mechanic' then
        ...
        TriggerClientEvent('QBCore:Notify', source, 'Yetkiniz yok', 'error')
    end
end)
...
QBCore.Functions.CreateCallback('qb-phone:server:GetInvoicesAll', function(source, cb)
    ...
    local niltype = 0
	if Player then
		MySQL.Async.fetchAll('SELECT SUM(amount) AS "amount" FROM phone_invoices WHERE citizenid = ?',{Player.PlayerData.citizenid}, function(data)
...
RegisterNetEvent('qb-phone:server:GiveInvoice', function()
	...
	local Player = QBCore.Functions.GetPlayer(src)
    if Player then
        MySQL.Async.insert('INSERT INTO phone_invoices (citizenid, amount, society, sender, sendercitizenid) VALUES (?, ?, ?, ?, ?)',{Player.PlayerData.citizenid, 10, "Garaj", "Vale", "Garaj"})
