...
AddEventHandler("nakres_videocall:sendData", function(data)
    ...
    if data.type == "store_user" then
        if user then return print(("[videocall]: Bu kişi zaten bir aktif bir görüntülü konuşmada | USER ID : %s"):format(data.callId)) end
        local newUser = {
    ...
    if not user then return print(("[videocall]: Arayan kişinin ice datası bulunamadı | USER ID : %s"):format(data.callId)) end
