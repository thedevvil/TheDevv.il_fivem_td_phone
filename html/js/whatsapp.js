...
QB.Phone.Functions.GetLastMessage = function(messages) {
    ...
    $.each(messages[messages.length - 1], function(i, msg){
        ...
        if(LastMessageData.message == '') 'Bunu yapamazsın'
    });
...
$(document).on('click', '#whatsapp-openedchat-send', function(e){
    ...
    if (Message !== null && Message !== undefined && Message !== "") {
        ...
        QB.Phone.Notifications.Add("fab fa-whatsapp", "Whatsapp", "Boş mesaj gönderemezsiniz!", "#25D366", 1750);
    }
});


$(document).on('focus', '.emojionearea-editor', function(){
    $.post('https://qb-phone/stopwalk');
});

$(document).on('focusout', '.emojionearea-editor', function(){
    $.post('https://qb-phone/startwalk');
});



$(document).on('keydown', function (e) {
    //$("#whatsapp-openedchat-message").show();
    $("#whatsapp-openedchat-message").val($(".emojionearea-editor").text());
    if (OpenedChatData.number !== null) {
        if(e.which === 13){
            $(".whatsapp-openedchat-messages").focus();
