...
QB.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(QB.Phone.Data.PlayerData.charinfo.firstname+"."+QB.Phone.Data.PlayerData.charinfo.lastname+"@mail.com");
    $("#mail-header-lastsync").html("Son Güncelleme "+MessageTime);
    if (Mails !== null && Mails !== undefined) {
        if (Mails.length > 0) {
            ...
            $(".mail-list").html('<p class="nomails">Mail kutunuz boş..</p>');
        }
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        QB.Phone.Notifications.Add("fas fa-ad", "Sarı Sayfalar", "Boş bir ilan giremezsiniz", "#ff8f1a", 2000);
    }
});
...
QB.Phone.Functions.RefreshAdverts = function(Adverts) {
    ...
    if (Adverts.length > 0 || Adverts.length == undefined) {
        ...
        $(".advert-list").html("");
        var element = '<div class="advert"><span class="advert-sender">Henüz ilan paylaşılmadı!</span></div>';
        $(".advert-list").append(element);
    }
}
...
$(document).on('click','#adv-delete',function(e){
    ...
    $.post('https://qb-phone/DeleteAdvert', function(){
        setTimeout(function(){
            QB.Phone.Notifications.Add("fas fa-ad", "Sarı Sayfalar", "İlan silinmiştir", "#ff8f1a", 2000);
        },400)
    });
})