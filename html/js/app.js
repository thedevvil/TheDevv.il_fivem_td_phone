...
$(document).on('click', '.phone-application', function(e){
    ...
    if (AppObject.length !== 0) {
        if (CanOpenApp) {
            if (QB.Phone.Data.currentApplication == null) {
                ...
                if (PressedApplication == "settings") {
                    ...
                    $("#mySerialNumber").text(QB.Phone.Data.PlayerData.metadata["phonedata"].SerialNumber);
                    ...
                    $.post('https://qb-phone/GetPlayerKeys', JSON.stringify({}), function(Keys){
                        ...
                        if (Keys.length > 0) {
                            $.each(Keys, function(i, key){
                                var elem = '<div class="mykeys-key" id="keyid-'+i+'"><span class="mykeys-key-label">' + key.HouseData.adress + '</span> <span class="mykeys-key-sub">Konumu GPS Ayarla</span> </div>';
        ...
        if (PressedApplication != null){
            QB.Phone.Notifications.Add("fas fa-exclamation-circle", "Sistem", QB.Phone.Data.Applications[PressedApplication].tooltipText+" mevcut değil!")
        }
    }
});
...
$(document).on('click', '.phone-tab-button', function(event){ //ana menü tuşu
    event.preventDefault();
        ...
        if (HasPhone) {
            ...
            if (QB.Phone.Notifications.Timeout == undefined || QB.Phone.Notifications.Timeout == null) {
                if (color != null || color != undefined) {
                    ...
                    $(".notification-title").css({"color":"#fff"});
    ...
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            ...
            case "UpdateCallTime":
                ...
                $(".phone-currentcall-title").html("Çağrı devam ediyor.. ("+timeString+")");
                break;
